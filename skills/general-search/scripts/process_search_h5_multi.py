...
def list_input_files(folder: Path, prefix: str | None):
    ...
    if prefix:
        files = [p for p in files if prefix in p.name]
    return sorted(files, key=lambda p: p.stat().st_mtime)


def main():
    in_dir = Path.home() / 'Desktop' / '搜索文件夹' / '通用h5'
    out_dir = Path.home() / 'Desktop' / '搜索文件夹' / '通用处理文件夹'
    out_dir.mkdir(parents=True, exist_ok=True)

    # 支持两种调用方式：
    # 1. python process_search_h5_multi.py <关键词前缀>  → 模糊匹配目录
    # 2. python process_search_h5_multi.py <目录路径>   → 处理指定目录（多省合并模式）
    # 3. python process_search_h5_multi.py               → 处理 in_dir 下所有子目录（排除 _MERGE_TEMP）
    ...
    if arg1:
        ...
        if arg_path.is_dir():
            # 传入的是目录路径：直接处理该目录
            in_dir = arg_path
            html_files = list_input_files(in_dir, None)
        else:
            # 作为关键词前缀模糊匹配
            candidates = sorted(in_dir.glob(f'*{arg1}*'), key=lambda p: p.stat().st_mtime, reverse=True)
            if candidates:
                in_dir = candidates[0]
            html_files = list_input_files(in_dir, arg1)
    else:
        # 无参数：处理 in_dir 下所有子目录的所有 .md 文件（多省合并模式）
        html_files = []
        for sub in sorted(in_dir.iterdir(), key=lambda p: p.stat().st_mtime, reverse=True):
            if sub.is_dir() and sub.name != '_MERGE_TEMP':
                html_files.extend(list_input_files(sub, None))
    if not html_files:
    ...
    # URL 过滤（去掉列表页/索引页/附件链接）
    before_filter = len(dedup)
    dedup = [r for r in dedup if is_valid_url(r['final_url'])]
    filtered_count = before_filter - len(dedup)
    if filtered_count > 0:
        print(f'  URL过滤掉 {filtered_count} 条（列表/索引/附件），剩余 {len(dedup)} 条')
