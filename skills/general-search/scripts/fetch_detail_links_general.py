...
def is_js_rendered(body: str) -> bool:
    """检测正文是否疑似 JS 渲染/登录限制页（正文抓到了外壳，正文内容在 JS 里）"""
    if not body:
        return False
    # JS 渲染页的典型 UI 提示文本
    js_patterns = [
        '收起', '展开', '打开网易新闻', '查看更多', '查看更多视频',
        '报料热线', '新闻报料', '报料邮箱', '全文共',
        '下载客户端', '扫码下载', '一键分享',
        '本文地址', '复制链接', '微信扫码',
        'article-detail', 'news-detail', 'content-detail',
        '打开凤凤凰新闻', '打开新浪新闻',
    ]
    short_js_patterns = [
        '打开网易新闻', '查看更多', '报料热线', '新闻报料',
        '打开.*新闻', '查看更多.*', '全文共.*字',
    ]
    ...
    for pat in js_patterns:
        if pat.lower() in body_lower:
            return True
    # 正文极短（<300字）且包含"打开"、"查看"类碎片文字
    if len(body.strip()) < 300:
        for pat in short_js_patterns:
...
def status_and_fallback(row: dict) -> tuple[str, str, str]:
    ...
    if poor and body_len == 0:
        status = 'POOR'
    # JS 渲染页单独标记，不归为 PARTIAL
    if status != 'GOOD' and is_js_rendered(body):
        status = 'JS渲染'
    need_browser = '是' if status in {'PARTIAL', 'POOR', 'JS渲染'} else '否'
...
def main():
    ...
    link_md = Path.home() / 'Desktop' / '搜索文件夹' / '通用处理文件夹' / '原文链接.md'
