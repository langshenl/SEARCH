...
description: 使用 Exa 搜索并按 v5 政策搜索技能的最终 9 字段格式输出结果。适用于"用 Exa 搜索政策/机构/主题内容""按 v5 格式返回""保存到桌面 exa搜索文件夹""导出 Excel""搜索湖北省…""搜索全国…"这类请求。支持自然语言输入，自动调用 Exa 搜索工具并直接产出 Excel，返回全部有效结果（无条数限制）。
---
...
- **关键词**：直接使用用户输入内容作为搜索词
- **条数**：返回全部有效结果（URL 验证后无 404 的全部保留）
